...
def capture_conclusion():
    ...
    try:
        ...
        sections = driver.find_elements(By.XPATH, "//span[text()='/ CONCLUSION']")
        if not sections:
            print("ERROR: Conclusion heading not found!")
            return
            
        print("Scrolling to Conclusion heading...")
        driver.execute_script("arguments[0].scrollIntoView({behavior: 'instant', block: 'center'});", sections[0])
        time.sleep(2)
        
        # Save screenshot
        screenshot_path = os.path.join(r"c:\Users\ASUS\Desktop\new portfolio\scratch", "cendrol_conclusion.png")
